...
def remove_from_chat_treasury(chat_id: int, amount: int) -> bool:
    t = get_chat_treasury(chat_id)
    if not t or t["balance"] < amount: return False
    cur.execute("UPDATE chat_treasury SET balance = balance - ? WHERE chat_id = ?", (amount, chat_id))
    conn.commit(); return True

def is_user_rewarded(chat_id: int, user_id: int) -> bool:
    return cur.execute("SELECT 1 FROM treasury_rewards WHERE chat_id = ? AND user_id = ?", (chat_id, user_id)).fetchone() is not None

def mark_user_rewarded(chat_id: int, user_id: int, invited_by: int):
    cur.execute("INSERT OR IGNORE INTO treasury_rewards (chat_id, user_id, invited_by) VALUES (?, ?, ?)", (chat_id, user_id, invited_by))
    conn.commit()
        ...
        if args[1].startswith("ref_"):
            ...
            if ref and ref[0] != uid:
                if not cur.execute("SELECT 1 FROM referrals WHERE referrer_id = ? AND referred_id = ?", (ref[0], uid)).fetchone():
                    ...
                    cur.execute("UPDATE users SET total_refs = total_refs + 1, ref_earned = ref_earned + ?, referrer_id = ? WHERE user_id = ?", (REF_REWARD, ref[0], uid))
...
async def bonus_cmd(msg: Message):
    ...
    if last:
        try:
            ...
            last_dt = None
        if last_dt and datetime.now() - last_dt < timedelta(days=1):
