...
class GameStates(StatesGroup):
    dice_waiting_bet = State()
    dice_waiting_chance = State()
    crash_waiting_bet = State()
    roulette_waiting_bet = State()
    slots_waiting_bet = State()
    coin_waiting_bet = State()
    coin_waiting_choice = State()
    hilo_waiting_bet = State()
    donate_custom = State()
    admin_give = State()
    admin_take = State()
    admin_ban = State()
...
@dp.message(F.text == "◀️ Назад в игры")
...
async def dice_start(message: Message, state: FSMContext):
    await state.set_state(GameStates.dice_waiting_bet)
    await message.answer(
        f"🎲 <b>Dice</b>\n\n"
...
@dp.message(GameStates.dice_waiting_bet)
async def dice_bet(message: Message, state: FSMContext):
    ...
    await state.set_state(GameStates.dice_waiting_chance)
    await message.answer(
        "🎲 <b>Выбери шанс:</b>\n"
        "• 50 - 50% (2x)\n"
        "• 33 - 33% (3x)\n"
...
@dp.message(GameStates.dice_waiting_chance)
async def dice_chance(message: Message, state: FSMContext):
    try:
...
async def crash_start(message: Message, state: FSMContext):
    await state.set_state(GameStates.crash_waiting_bet)
    await message.answer(
        f"🚀 <b>Crash</b>\n\n"
        f"💰 Введите ставку (от {MIN_BET} до {MAX_BET} POCX):",
        reply_markup=cancel_reply_keyboard()
...
@dp.message(GameStates.crash_waiting_bet)
async def crash_play(message: Message, state: FSMContext):
    ...
    # Игрок всегда выигрывает с множителем 1.5x (можно менять)
...
async def roulette_start(message: Message, state: FSMContext):
    await state.set_state(GameStates.roulette_waiting_bet)
    await message.answer(
        "🎡 <b>Рулетка</b>\n\n"
...
@dp.message(GameStates.roulette_waiting_bet)
...
async def slots_start(message: Message, state: FSMContext):
    await state.set_state(GameStates.slots_waiting_bet)
    await message.answer(
        "🎰 <b>Слоты</b>\n\n"
        "Таблица выплат:\n"
...
@dp.message(GameStates.slots_waiting_bet)
...
async def coin_start(message: Message, state: FSMContext):
    await state.set_state(GameStates.coin_waiting_choice)
    await message.answer(
        "🪙 <b>Coin Flip</b>\n\n"
...
@dp.message(GameStates.coin_waiting_choice)
async def coin_input(message: Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await state.clear()
        await message.answer("❌ Игра отменена", reply_markup=games_reply_keyboard())
        return
...
async def hilo_start(message: Message, state: FSMContext):
    await state.set_state(GameStates.hilo_waiting_bet)
    await message.answer(
        "🃏 <b>Hi-Lo</b>\n\n"
        "Правила:\n"
...
@dp.message(GameStates.hilo_waiting_bet)
...
async def main():
    ...
    print("✅ Reply-клавиатура для удобства")
    print("✅ Все 6 игр активны: Dice, Crash, Roulette, Slots, CoinFlip, Hi-Lo")
    print("✅ Пополнение Stars работает")
