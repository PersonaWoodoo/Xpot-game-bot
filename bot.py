...
# Премиум эмоджи для текста (работают только в сообщениях)
PREMIUM_MONEY = '<tg-emoji emoji-id="5283232570660634549">💎</tg-emoji>'
PREMIUM_BACK = '<tg-emoji emoji-id="5960671702059848143">🔙</tg-emoji>'

# Обычные эмоджи для кнопок
EMOJI_MONEY = "💰"
EMOJI_BACK = "◀️"
...
def subscription_keyboard(not_subscribed: list) -> InlineKeyboardMarkup:
    ...
    kb.append([InlineKeyboardButton(text=f"{EMOJI_BACK} Проверить подписку", callback_data="check_subscription")])
...
def main_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=f"{EMOJI_MONEY} Финансы", callback_data="finance")],
...
def games_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=f"{EMOJI_BACK} Назад", callback_data="main")]
    ])
...
def dice_menu():
    ...
    kb.add(InlineKeyboardButton(text=f"{EMOJI_BACK} Назад", callback_data="games"))
    return kb
...
def roulette_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=f"{EMOJI_BACK} Назад", callback_data="games")]
    ])
...
def crash_control():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=f"{EMOJI_BACK} Выход", callback_data="games")]
    ])
...
def hilo_controls(can_cashout: bool = False):
    ...
    if can_cashout:
        kb.inline_keyboard.append([InlineKeyboardButton(text=f"{EMOJI_MONEY} Забрать", callback_data="hilo_cashout")])
    return kb
...
def finance_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"{EMOJI_MONEY} Пополнить (Stars)", callback_data="donate")],
        [InlineKeyboardButton(text=f"{EMOJI_BACK} Назад", callback_data="main")]
    ])
...
def admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=f"{EMOJI_MONEY} Выдать POCX", callback_data="admin_give")],
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        f"{PREMIUM_MONEY} Баланс: <b>{user['balance']:,} POCX</b>\n"
        f"💎 VIP: {vip}\n\n"
...
async def cb_main(callback: CallbackQuery):
    ...
    vip, _ = get_vip(user["total_wagered"])
    await callback.message.edit_text(
        f"🎰 <b>Главное меню</b>\n\n{PREMIUM_MONEY} Баланс: <b>{user['balance']:,} POCX</b> | {vip}",
        reply_markup=main_menu()
    )
    await callback.answer()
...
async def cb_profile(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"{PREMIUM_MONEY} Баланс: {user['balance']:,} POCX\n"
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{EMOJI_BACK} Назад", callback_data="main")]])
    )
...
async def cb_stats(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"📊 ROI: {roi:.1f}%\n"
        f"{PREMIUM_MONEY} Общий вейджер: {user['total_wagered']:,} POCX\n"
...
async def cb_referrals(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"🔗 Ваша ссылка:\n<code>{link}</code>\n\n"
        f"👤 Рефералов: {count}\n"
        f"{PREMIUM_MONEY} Заработано: {earned:,} POCX\n\n"
        f"🎁 За каждого друга: +{REF_REWARD:,} POCX",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{EMOJI_BACK} Назад", callback_data="main")]])
    )
...
async def cb_daily(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"{PREMIUM_MONEY} +{bonus:,} POCX\n"
        f"🔥 Серия дней: {streak + 1}\n"
        f"📈 Бонус увеличен на {min(streak * 5, 50)}%!",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{EMOJI_BACK} Назад", callback_data="bonuses")]])
    )
...
async def cb_welcome(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"🎁 <b>Приветственный бонус!</b>\n\n{PREMIUM_MONEY} +{WELCOME_BONUS:,} POCX",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text=f"{EMOJI_BACK} Назад", callback_data="bonuses")]])
    )
...
async def cb_promo(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🔑 <b>Введите промокод</b>")
    await callback.answer()
...
async def promo_use(message: Message, state: FSMContext):
    ...
    await message.answer(f"✅ Промокод активирован!\n{PREMIUM_MONEY} +{bonus:,} POCX", reply_markup=main_menu())
    await state.clear()
...
async def cb_top_all(callback: CallbackQuery):
    ...
    await callback.message.edit_text("\n".join(lines), reply_markup=top_menu())
    await callback.answer()
...
async def cb_finance(callback: CallbackQuery):
    await callback.message.edit_text(f"{PREMIUM_MONEY} <b>Финансы</b>", reply_markup=finance_menu())
    await callback.answer()

@dp.callback_query(F.data == "donate")
async def cb_donate(callback: CallbackQuery):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"⭐ 1 Star - 2,500 POCX", callback_data="donate_1")],
        [InlineKeyboardButton(text=f"⭐⭐ 5 Stars - 12,500 POCX", callback_data="donate_5")],
        [InlineKeyboardButton(text=f"⭐⭐⭐ 10 Stars - 25,000 POCX", callback_data="donate_10")],
        [InlineKeyboardButton(text=f"💰 25 Stars - 62,500 POCX", callback_data="donate_25")],
        [InlineKeyboardButton(text=f"💎 50 Stars - 125,000 POCX", callback_data="donate_50")],
        [InlineKeyboardButton(text=f"✏️ Своя сумма", callback_data="donate_custom")],
        [InlineKeyboardButton(text=f"{EMOJI_BACK} Назад", callback_data="finance")]
    ])
    await callback.message.edit_text(
...
async def process_donate(callback: CallbackQuery, state: FSMContext):
    if callback.data == "donate_custom":
        ...
        await callback.message.edit_text(f"{PREMIUM_MONEY} Введи количество Stars (1-1000):")
        await callback.answer()
        return
...
async def successful_payment(message: Message):
    ...
    await message.answer(
        f"✅ Оплачено {stars} ⭐!\n{PREMIUM_MONEY} Получено {pocx:,} POCX\n\n⭐ 1 Star = 2500 POCX",
        reply_markup=main_menu()
    )
...
async def dice_bet(message: Message, state: FSMContext):
    ...
    if win:
        msg = f"🎲 <b>Dice</b>\n\nВыпало: {r} | Цель: <{tgt}\n✅ ВЫИГРЫШ!\n{PREMIUM_MONEY} Ставка: {bet:,} POCX\n🎉 Выигрыш: {win_amount:,} POCX ({multi}x)\n\n{PREMIUM_MONEY} Баланс: {user['balance']:,} POCX"
    else:
        msg = f"🎲 <b>Dice</b>\n\nВыпало: {r} | Цель: <{tgt}\n❌ ПРОИГРЫШ\n{PREMIUM_MONEY} Ставка: {bet:,} POCX\n😢 Вы проиграли\n\n{PREMIUM_MONEY} Баланс: {user['balance']:,} POCX"
...
async def cb_crash(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"🚀 <b>Crash</b>\n\n{PREMIUM_MONEY} Введите ставку (от 100 до 500,000 POCX):")
    await callback.answer()
...
async def crash_animation(uid: int, msg: Message, bet: int, cp: float, state: FSMContext):
    ...
    while uid in crash_games and crash_games[uid].get("is_active", False):
        ...
        try:
            ...
            await msg.edit_text(
                f"🚀 <b>{cur:.2f}x</b>\n\n[{bar}]\n\n{PREMIUM_MONEY} {bet:,} → 💰 {int(bet * cur):,} POCX",
                reply_markup=crash_control()
            )
        except:
            pass
...
async def crash_cashout(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"🎉 <b>ВЫИГРЫШ!</b>\n\n📈 Забрано на <b>{cur:.2f}x</b>\n{PREMIUM_MONEY} Ставка: {game['bet']:,} POCX\n{PREMIUM_MONEY} Выигрыш: <b>{win:,} POCX</b>",
        reply_markup=main_menu()
    )
    crash_games.pop(uid)
    await callback.answer()
...
async def roulette_choice(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"🎡 <b>Рулетка - {labels[bet_type]}</b>\n\n{PREMIUM_MONEY} Введите ставку (от {MIN_BET:,} до {MAX_BET:,} POCX):"
    )
    await callback.answer()
...
async def roulette_bet(message: Message, state: FSMContext):
    ...
    win_text = f"{PREMIUM_MONEY} Выигрыш: {win_amount:,} POCX ({multiplier}x)" if win else "😢 Вы проиграли"
...
async def cb_slots(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "🍇🍇🍇 = 10x | 🍊🍊🍊 = 8x | 🍋🍋🍋 = 5x | 🍒🍒🍒 = 3x\n"
        "Два одинаковых = 1.5x\n\n"
        f"{PREMIUM_MONEY} Введите ставку (от {MIN_BET:,} до {MAX_BET:,} POCX):"
    )
    await callback.answer()
...
async def slots_bet(message: Message, state: FSMContext):
    ...
    await spin_msg.edit_text(
        ...
        f"{f'{PREMIUM_MONEY} Выигрыш: {win_amount:,} POCX ({multiplier}x)' if win_amount > 0 else '😢 Вы проиграли'}\n\n"
        f"{PREMIUM_MONEY} Баланс: {user['balance']:,} POCX",
        reply_markup=main_menu()
    )
    await state.clear()
...
async def coin_bet(message: Message, state: FSMContext):
    ...
    await flip_msg.edit_text(
        ...
        f"{'🎉 ВЫИГРЫШ!' if win else '❌ ПРОИГРЫШ'}\n"
        f"{f'{PREMIUM_MONEY} Выигрыш: {win_amount:,} POCX (2x)' if win else '😢 Вы проиграли'}\n\n"
        f"{PREMIUM_MONEY} Баланс: {user['balance']:,} POCX",
        reply_markup=main_menu()
    )
    await state.clear()
...
async def hilo_guess(callback: CallbackQuery, state: FSMContext):
    ...
    if win:
        ...
        await callback.message.edit_text(
            ...
            f"{PREMIUM_MONEY} Можно забрать: <b>{int(game['bet'] * game['multiplier']):,} POCX</b>\n\n"
            f"Продолжаем?",
        ...
        await callback.message.edit_text(
            ...
            f"{PREMIUM_MONEY} Проигрыш: {game['bet']:,} POCX\n"
            f"{PREMIUM_MONEY} Баланс: {user['balance']:,} POCX",
            reply_markup=main_menu()
        )
...
async def hilo_cashout(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"🃏 <b>Hi-Lo - ВЫИГРЫШ!</b>\n\n"
        f"📈 Множитель: <b>{game['multiplier']}x</b>\n"
        f"{PREMIUM_MONEY} Ставка: {game['bet']:,} POCX\n"
        f"{PREMIUM_MONEY} <b>Выигрыш: {win:,} POCX</b>\n\n"
        f"{PREMIUM_MONEY} Баланс: {user['balance']:,} POCX",
        reply_markup=main_menu()
    )
    hilo_games.pop(uid, None)
...
async def cb_admin_stats(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"🚫 Забанено: {banned}\n"
        f"{PREMIUM_MONEY} Общий баланс: {total_balance:,} POCX\n"
...
async def cb_admin_give(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"{PREMIUM_MONEY} <b>Выдача POCX</b>\n\nВведи ID и сумму через пробел\nПример: `123456789 50000`")
    await callback.answer()
...
async def cb_admin_take(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"➖ <b>Списание POCX</b>\n\nВведи ID и сумму через пробел\nПример: `123456789 50000`")
    await callback.answer()
...
async def admin_promo_code(message: Message, state: FSMContext):
    ...
    await message.answer(f"{PREMIUM_MONEY} Введи сумму награды:")
...
async def admin_promo_uses(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"{PREMIUM_MONEY} Награда: {reward:,} POCX\n"
        f"🎯 Активаций: {uses}",
        reply_markup=admin_menu()
    )
    await state.clear()
...
async def main():
    ...
    print("✅ Премиум эмоджи добавлены в тексты сообщений")
