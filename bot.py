...
CREATE TABLE IF NOT EXISTS users (
    ...
    games_played_today INTEGER DEFAULT 0, custom_status TEXT,
    mute_games_until INTEGER DEFAULT 0, mute_chat_until INTEGER DEFAULT 0,
    total_donated INTEGER DEFAULT 0, total_refs INTEGER DEFAULT 0,
    ref_earned INTEGER DEFAULT 0, last_command_time INTEGER DEFAULT 0
);
...
def fmt_money(v: int) -> str:
    if v < 0:
        ...
        sign = ""
    if v >= 1_000_000_000: return f"{sign}{v/1_000_000_000:.1f}ккк POCX"
    if v >= 1_000_000: return f"{sign}{v/1_000_000:.1f}кк POCX"
    if v >= 1_000: return f"{sign}{v/1_000:.1f}к POCX"
    return f"{sign}{v} POCX"
...
def get_user(uid: int):
    ...
    return {
        ...
        "referrer_id": r[7], "last_daily_bonus": r[8],
        "daily_streak": r[9] or 0, "games_played_today": r[10] or 0,
        "custom_status": r[11], "mute_games_until": r[12] or 0,
        "mute_chat_until": r[13] or 0, "total_donated": r[14] or 0,
        "total_refs": r[15] or 0, "ref_earned": r[16] or 0,
        "last_command_time": r[17] or 0
    }
...
def record_game(uid, game_type, bet, mult, win, result, details):
    ...
    # Начисление реферальных отчислений (2% от проигрыша)
    u = get_user(uid)
    if u and u.get("referrer_id"):
        ref_id = u["referrer_id"]
        if result == "loss":
            ref_bonus = int(bet * REF_PERCENT)
            add_pocx(ref_id, ref_bonus)
            cur.execute("UPDATE referrals SET earned_amount = earned_amount + ? WHERE referrer_id = ? AND referred_id = ?",
                       (ref_bonus, ref_id, uid))
            cur.execute("UPDATE users SET ref_earned = ref_earned + ? WHERE user_id = ?", (ref_bonus, ref_id))
            conn.commit()
...
async def start(msg: Message):
        ...
        if args[1].startswith("ref_"):
            ...
            if ref and ref[0] != uid:
                if not cur.execute("SELECT 1 FROM referrals WHERE referrer_id = ? AND referred_id = ?", (ref[0], uid)).fetchone():
                    ...
                    cur.execute("UPDATE users SET referrer_id = ? WHERE user_id = ?", (ref[0], uid))
                    conn.commit()
    ...
    await msg.answer(
        ...
        f"⏱ Кулдаун между командами: 5 сек.",
...
async def help_cmd(msg: Message):
    ...
    text2 = (
        ...
        "<b>🎟 Промокод:</b> <code>промокод КОД</code>\n\n"
        "💡 <b>Сокращения:</b> 1к=1000, все=весь баланс, пол=половина\n"
        "⏱ <b>Кулдаун:</b> 5 секунд между командами"
    )
    try:
...
# Единый обработчик для добавления бота и новых участников
@dp.chat_member()
async def on_chat_member_update(event: ChatMemberUpdated):
    ...
    # Новый участник в чате (для казны)
    if event.new_chat_member.status in ["member", "administrator", "creator"]:
        ...
        cur.execute("SELECT * FROM chat_treasury WHERE chat_id = ? AND is_active = 1", (chat_id,))
        treasury = cur.fetchone()
        if not treasury:
            return
        
        cur.execute("SELECT 1 FROM treasury_rewards WHERE chat_id = ? AND user_id = ?", (chat_id, new_user_id))
        if cur.fetchone():
            return
        ...
        reward = treasury[3]  # reward_per_invite
        if treasury[2] < reward:  # balance < reward
            return
        
        cur.execute("UPDATE chat_treasury SET balance = balance - ? WHERE chat_id = ?", (reward, chat_id))
        add_pocx(inviter_id, reward)
        cur.execute("INSERT OR IGNORE INTO treasury_rewards (chat_id, user_id, invited_by) VALUES (?, ?, ?)",
                   (chat_id, new_user_id, inviter_id))
        conn.commit()
...
@dp.message(lambda m: m.text and m.text.lower().strip() in ["профиль", "п"])
...
@dp.message(lambda m: m.text and m.text.lower().strip() in ["топ", "т"])
...
@dp.message(lambda m: m.text and m.text.lower().strip() in ["ст", "стата", "статистика"])
...
# -------------------- БОНУС --------------------
@dp.message(lambda m: m.text and m.text.lower().strip() in ["бонус", "бн"])
...
async def bonus_cmd(msg: Message):
    ...
    if not bio_ok:
        await msg.answer(
            ...
            "ℹ️ Чтобы получать ежедневные подарки, необходимо выполнить указанные выше условия."
        )
        return
    ...
    if last:
        try:
            ...
            if datetime.now() - last_dt < timedelta(days=1):
                await msg.answer("❌ Бонус уже получен сегодня!")
                return
        except:
            pass
    ...
    add_pocx(uid, bonus)
    cur.execute("UPDATE users SET last_daily_bonus = ?, daily_streak = daily_streak + 1 WHERE user_id = ?", (datetime.now().isoformat(), uid))
    conn.commit()
...
async def donate_custom_amount(msg: Message, state: FSMContext):
    try: stars = int(msg.text); assert 1 <= stars <= 2500
...
async def close_menu(callback: CallbackQuery): await callback.message.delete(); await callback.answer()
...
async def bank_menu(msg: Message):
    ...
    await msg.answer(
        ...
        "• 30 дней: +5%\n\n"
        "📋 <b>Команды:</b>\n"
        "<code>банк открыть [сумма] [7/14/30]</code>\n"
        "<code>банк список</code>\n"
        "<code>банк снять [ID/все]</code> — досрочно (-10%)\n"
...
# -------------------- РЕФЕРАЛЫ --------------------
@dp.message(lambda m: m.text and m.text.lower().strip() in ["рефералы", "реф", "рефка"])
...
async def roulette_game(msg: Message):
    ...
    if not u: return
    parts = msg.text.split()
    if len(parts) < 3: await msg.reply("❌ <code>рул 1000 красное</code>"); return
    try: bet = parse_amount(parts[1], u["balance"]); assert MIN_BET <= bet <= u["balance"]
    ...
    number = 0 if random.random() < 0.027 else random.randint(1, 36)
...
async def crash_game(msg: Message):
    ...
    if not u: return
    parts = msg.text.split()
    if len(parts) < 3: await msg.reply("❌ <code>краш 1000 2</code>"); return
    try: bet = parse_amount(parts[1], u["balance"]); target = float(parts[2].replace(",",".")); assert MIN_BET <= bet <= u["balance"] and 1.01 <= target <= 10
...
async def cube_game(msg: Message):
    ...
    if not u: return
    parts = msg.text.split()
    if len(parts) < 3: await msg.reply("❌ <code>кубик 1000 5</code>"); return
    try: bet = parse_amount(parts[1], u["balance"]); assert MIN_BET <= bet <= u["balance"]
    ...
    dice = await msg.answer_dice(emoji="🎲"); number = dice.dice.value
    win = False; mult = 0
...
async def dice_game(msg: Message):
    ...
    if not u: return
    parts = msg.text.split()
    if len(parts) < 3: await msg.reply("❌ <code>кости 1000 м</code>"); return
    try: bet = parse_amount(parts[1], u["balance"]); assert MIN_BET <= bet <= u["balance"]
    except: await msg.reply(f"❌ Ставка"); return
    choice = parts[2].lower()
...
async def football_game(msg: Message):
    ...
    if not u: return
    parts = msg.text.split()
    if len(parts) < 2: await msg.reply("❌ <code>футбол 1000 гол</code>"); return
    try: bet = parse_amount(parts[1], u["balance"]); assert MIN_BET <= bet <= u["balance"]
    except: await msg.reply(f"❌ Ставка"); return
...
async def basketball_game(msg: Message):
    ...
    if not u: return
    parts = msg.text.split()
    if len(parts) < 2: await msg.reply("❌ <code>баскет 1000</code>"); return
    try: bet = parse_amount(parts[1], u["balance"]); assert MIN_BET <= bet <= u["balance"]
    except: await msg.reply(f"❌ Ставка"); return
    if not remove_pocx(msg.from_user.id, bet): await msg.reply("❌ Недостаточно средств"); return
...
async def darts_game(msg: Message):
    ...
    if not u: return
    parts = msg.text.split()
    if len(parts) < 3: await msg.reply("❌ <code>дартс 1000 центр</code>"); return
    try: bet = parse_amount(parts[1], u["balance"]); assert MIN_BET <= bet <= u["balance"]
    except: await msg.reply(f"❌ Ставка"); return
    choice = parts[2].lower()
...
async def slots_game(msg: Message):
    ...
    if not u: return
    parts = msg.text.split()
    if len(parts) < 2: await msg.reply("❌ <code>слоты 1000</code>"); return
    try: bet = parse_amount(parts[1], u["balance"]); assert MIN_BET <= bet <= u["balance"]
    except: await msg.reply(f"❌ Ставка"); return
    if not remove_pocx(msg.from_user.id, bet): await msg.reply("❌ Недостаточно средств"); return
...
async def coinflip_game(msg: Message):
    ...
    if not u: return
    parts = msg.text.split()
    if len(parts) < 3: await msg.reply("❌ <code>монетка 1000 орел</code>"); return
    try: bet = parse_amount(parts[1], u["balance"]); assert MIN_BET <= bet <= u["balance"]
    except: await msg.reply(f"❌ Ставка"); return
    choice = parts[2].lower()
...
async def rps_game(msg: Message):
    ...
    if not u: return
    parts = msg.text.split()
    if len(parts) < 3: await msg.reply("❌ <code>кнб 1000 камень</code>"); return
    try: bet = parse_amount(parts[1], u["balance"]); assert MIN_BET <= bet <= u["balance"]
    except: await msg.reply(f"❌ Ставка"); return
    choice = parts[2].lower()
...
# -------------------- ПЕРЕДАЧА --------------------
@dp.message(lambda m: m.reply_to_message and m.text and m.text.lower().split()[0] in ("передать", "п", "дать", "накинуть"))
async def transfer(msg: Message):
    if not check_cooldown(msg.from_user.id): return
    parts = msg.text.split()
    if len(parts) < 2: await msg.reply("❌ Укажите сумму"); return
    try: amount = parse_amount(parts[1]); assert amount >= 100
    except: await msg.reply("❌ Сумма от 100"); return
    u = get_user(msg.from_user.id)
    if not u: return
    fee = int(amount * TRANSFER_FEE); total = amount + fee
...
async def check_create(msg: Message, state: FSMContext):
    if not check_cooldown(msg.from_user.id): return
    await state.set_state(GameStates.check_create_amount)
    await msg.reply("💰 Введите сумму на одну активацию чека:")

@dp.message(GameStates.check_create_amount)
async def check_create_amount_input(msg: Message, state: FSMContext):
    try:
        amt = parse_amount(msg.text)
        if amt <= 0 or amt > 5_000_000: raise ValueError
    except: await msg.reply("❌ Сумма 1-5,000,000"); return
    await state.update_data(check_amt=amt)
    await state.set_state(GameStates.check_create_count)
    await msg.reply("🔢 Введите количество активаций (1-50):")

@dp.message(GameStates.check_create_count)
async def check_create_count_input(msg: Message, state: FSMContext):
    try:
        cnt = int(msg.text)
        if cnt <= 0 or cnt > 50: raise ValueError
    except: await msg.reply("❌ Число 1-50"); return
    data = await state.get_data()
    amt = data["check_amt"]
    total = amt * cnt
    if not remove_pocx(msg.from_user.id, total): await msg.reply("❌ Недостаточно средств"); return
...
async def admin_give_start(callback: CallbackQuery, state: FSMContext):
    if not has_perm(callback.from_user.id): await callback.answer("❌", show_alert=True); return
...
async def clan_close(callback: CallbackQuery): await callback.message.delete(); await callback.answer()
...
async def clan_deposit_amount(msg: Message, state: FSMContext):
    ...
    await state.clear(); await msg.answer(f"✅ +{fmt_money(amt)} в казну клана")
...
async def clan_rename_exec(msg: Message, state: FSMContext):
    ...
    await state.clear(); await msg.answer(f"✅ Клан переименован в '{name}'!")
...
async def clan_role_exec(msg: Message, state: FSMContext):
    ...
    else: conn.commit(); await msg.answer("✅ Заместитель назначен!"); await state.clear()
...
async def clan_kick_exec(msg: Message, state: FSMContext):
    ...
    else: await msg.answer("❌ Не найден или владелец"); await state.clear()
...
async def clan_ban_exec(msg: Message, state: FSMContext):
    ...
    cur.execute("DELETE FROM clan_members WHERE user_id = ? AND clan_id = ? AND role != 'owner'", (target_id, clan_id))
    conn.commit(); await state.clear(); await msg.answer("✅ Забанен в клане!")
...
def add_to_chat_treasury(chat_id: int, amount: int):
    cur.execute("UPDATE chat_treasury SET balance = balance + ? WHERE chat_id = ?", (amount, chat_id))
    conn.commit()

# -------------------- ЗАПУСК --------------------
async def main():
    print("🤖 POCX Casino запущен!")
    print("✅ Игры, банк, кланы, рефералы, бонусы, казна")
