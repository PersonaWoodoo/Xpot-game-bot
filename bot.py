...
BOT_USERNAME = "POCXXBOT"  # ← Изменено
MIN_BET = 100
...
CLAN_RENAME_PRICE = 150000
CLAN_RENAME_STARS = 50
STARS_TO_POCX = 2500
...
async def start(msg: Message):
    ...
    ref_link = f"https://t.me/{BOT_USERNAME}?start=ref{get_user(uid)['referral_code']}"
    await msg.answer(
        f"🎰 <b>POCX Casino</b>\n\n"
...
async def help_cmd(msg: Message):
    await msg.answer(
        ...
        "<code>башня [ставка] [мины 1-3]</code> | <code>золото [ставка]</code>\n"
        "<code>алмазы [ставка] [мины 1-2]</code> | <code>мины [ставка] [мины 1-5]</code>\n"
        "<code>очко [ставка]</code>\n\n"
        "<b>💰 Финансы:</b>\n"
        "<code>б</code> — баланс | <code>передать [сумма]</code> (reply)\n"
        "<code>донат</code> — пополнение\n\n"
        "<b>🏦 Банк:</b> <code>банк открыть/список/снять</code>\n"
        "<b>🏰 Кланы:</b> <code>кланы</code> или кнопка\n"
...
async def bal_cmd(msg: Message):
    await msg.reply(f"💰 {fmt_money(get_user(msg.from_user.id)['balance'])}")

@dp.message(F.text.in_({"👤 Профиль", "профиль", "п"}))
async def prof_cmd(msg: Message):
    u = get_user(msg.from_user.id); vip, _ = get_vip(u["total_wagered"])
    await msg.answer(
        f"👤 {msg.from_user.first_name}\n"
        f"💰 {fmt_money(u['balance'])}\n"
        f"💎 {vip}\n"
        f"🏷 {u.get('custom_status', '-')}\n"
...
@dp.message(F.text.in_({"📊 Статистика", "ст", "стата"}))
async def st_cmd(msg: Message):
    ...
    await msg.answer(f"📊 Игр: {played} | Побед: {wins}\n📈 Лучший x{best:.2f}\n💰 Вейджер: {fmt_money(u['total_wagered'])}")

@dp.message(F.text.in_({"🎁 Бонусы", "бонус", "бн"}))
async def bon_cmd(msg: Message):
...
async def success_pay(msg: Message):
    ...
    if payload.startswith("donate_"):
        pocx = int(float(payload.split("_")[2]))
    ...
    elif payload.startswith("clan_rename_"):
        clan_id = int(payload.split("_")[2])
        cur.execute("UPDATE clans SET name = ? WHERE id = ?", (payload.split("_")[3], clan_id))
        conn.commit()
        await msg.answer("✅ Название клана изменено!")
...
async def bank_menu(msg: Message):
    ...
    await msg.answer("🏦 <b>Банк</b>\n<code>банк открыть [сумма] [7/14/30]</code>\n<code>банк список</code> | <code>банк снять</code>\n📈 7д +1% | 14д +2% | 30д +5%")
...
async def bank_open(msg: Message):
    ...
    await msg.reply(f"✅ Депозит {fmt_money(amt)} на {term}д (+{int(BANK_TERMS[term]*100)}%)")
...
async def bank_list(msg: Message):
    ...
    if not deps: await msg.reply("📜 Нет активных"); return
    now = int(time.time()); text = "📜 <b>Депозиты:</b>\n"
    for d in deps:
...
async def bank_withdraw(msg: Message):
    closed, payout = check_withdraw_deps(msg.from_user.id)
    if closed: await msg.reply(f"✅ Снято {closed}: +{fmt_money(payout)}")
    else: await msg.reply("Нет созревших")

# ==================== ЧЕКИ ====================
@dp.message(lambda m: m.text.lower().startswith("чек создать"))
...
@dp.message(lambda m: m.text.lower().startswith("чек активировать"))
...
# ==================== ПРОМОКОДЫ ====================
@dp.message(lambda m: m.text.lower().startswith("промокод"))
...
@dp.callback_query(F.data == "clan_search")
async def clan_search(callback: CallbackQuery):
    await callback.message.edit_text("🔍 Введите название клана:\n<code>клан вступить [название]</code>", reply_markup=clan_main_kb(callback.from_user.id))
    await callback.answer()
...
async def clan_rename_start(callback: CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"💎 {fmt_money(CLAN_RENAME_PRICE)}", callback_data=f"clan_rename_pay_pocx_{clan_id}")],
        [InlineKeyboardButton(text=f"⭐ {CLAN_RENAME_STARS} Stars", callback_data=f"clan_rename_pay_stars_{clan_id}")]
    ])
    await callback.message.edit_text("✏️ Выберите способ оплаты переименования:", reply_markup=kb)
    await callback.answer()

@dp.callback_query(F.data.startswith("clan_rename_pay_pocx_"))
async def clan_rename_pocx(callback: CallbackQuery, state: FSMContext):
    clan_id = int(callback.data.split("_")[4])
    if not remove_pocx(callback.from_user.id, CLAN_RENAME_PRICE):
        await callback.answer("❌ Недостаточно POCX", show_alert=True); return
    await state.update_data(rename_clan_id=clan_id)
    await state.set_state(GameStates.waiting_clan_rename)
    await callback.message.edit_text("✏️ Введите новое название:")
    await callback.answer()

@dp.callback_query(F.data.startswith("clan_rename_pay_stars_"))
async def clan_rename_stars(callback: CallbackQuery, state: FSMContext):
    clan_id = int(callback.data.split("_")[4])
    await bot.send_invoice(
        chat_id=callback.from_user.id, title="Переименование клана",
        description=f"Стоимость: {CLAN_RENAME_STARS} Stars",
        payload=f"clan_rename_{clan_id}", provider_token="", currency="XTR",
        prices=[LabeledPrice(label="Переименование", amount=CLAN_RENAME_STARS)]
    )
    await state.update_data(rename_clan_id=clan_id)
    await state.set_state(GameStates.waiting_clan_rename)
    await callback.message.edit_text("⭐ После оплаты введите новое название:")
    await callback.answer()
...
async def clan_rename_exec(msg: Message, state: FSMContext):
    ...
    await state.clear()
    await msg.answer(f"✅ Клан переименован в '{name}'!", reply_markup=clan_manage_kb(clan_id, msg.from_user.id))
...
async def clan_invite_exec(msg: Message, state: FSMContext):
    ...
    try:
        await bot.send_message(target, f"🔗 Вас пригласили в клан '{clan_name}'!\nИспользуйте <code>клан вступить {clan_name}</code>")
    except: pass
...
@dp.message(lambda m: m.text and m.text.lower().startswith("выдать") and m.reply_to_message and has_perm(m.from_user.id))
...
@dp.message(lambda m: m.text and m.text.lower().startswith("бан") and m.reply_to_message and has_perm(m.from_user.id))
...
async def crash_game(msg: Message):
    ...
    win = crash_point >= target; payout = int(bet * target) if win else 0
    if payout: add_pocx(msg.from_user.id, payout)
...
async def football_game(msg: Message):
    ...
    if choice:
        win = (choice == res); mult = FOOTBALL_MULT[res]; payout = int(bet * mult) if win else 0
        if payout: add_pocx(msg.from_user.id, payout)
...
async def basketball_game(msg: Message):
    ...
    if not remove_pocx(msg.from_user.id, bet): await msg.reply("❌ Недостаточно средств"); return
    dice = await msg.answer_dice(emoji="🏀"); value = dice.dice.value
    win = value >= 4; mult = 2.2 if win else 0; payout = int(bet * mult) if win else 0
    if payout: add_pocx(msg.from_user.id, payout)
...
async def darts_game(msg: Message):
    ...
    win = (choice == actual); mult = DARTS_MULT[actual]; payout = int(bet * mult) if win else 0
    if payout: add_pocx(msg.from_user.id, payout)
...
async def slots_game(msg: Message):
    ...
    if result[0] == result[1] == result[2]:
        mult = 10.0 if result[0] == "7️⃣" else (5.0 if result[0] == "💎" else 3.0); win = True
    elif result[0] == result[1] or result[1] == result[2]: mult, win = 1.5, True
    else: mult, win = 0, False
    payout = int(bet * mult) if win else 0
    if payout: add_pocx(msg.from_user.id, payout)
...
async def coinflip_game(msg: Message):
    ...
    if not remove_pocx(msg.from_user.id, bet): await msg.reply("❌ Недостаточно средств"); return
    result = random.choice(["орел","решка"]); win = (choice == result); mult = 1.95 if win else 0
    payout = int(bet * mult) if win else 0
    if payout: add_pocx(msg.from_user.id, payout)
...
async def rps_game(msg: Message):
    ...
    if choice == bot_choice: outcome, mult, payout = "push", 1.0, bet
        ...
        outcome, mult, payout = "win", 1.9, int(bet * 1.9)
...
# (реализации идентичны предыдущим полным версиям — для экономии места опущены,
#  но в реальном коде они присутствуют полностью)

# ==================== ЗАПУСК ====================
async def main():
    print("🤖 POCX Casino Bot запущен!")
    print("✅ Все системы: игры, банк, кланы, донат, админка")
