...
# ==================== БАЗА ДАННЫХ (НЕ УДАЛЯЕТСЯ ПРИ ЗАПУСКЕ) ====================
# База данных создаётся один раз и сохраняется между перезапусками
...
conn.commit()
print("✅ База данных подключена (данные сохраняются)")
...
async def dice_bet_amount(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "• 33 (33% / 3x)\n"
        "• 50 (50% / 2x)\n\n"
        "Введите число: 1, 5, 10, 25, 33 или 50"
    )
...
async def crash_play(message: Message, state: FSMContext):
    ...
    # Выигрыш с множителем на 10% ниже краша
    multiplier = round(crash_point * 0.9, 2)
...
async def roulette_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "• <code>руль красное 1000</code> - 2x\n"
        "• <code>руль черное 1000</code> - 2x\n"
        "• <code>руль чет 1000</code> - 2x\n"
        "• <code>руль нечет 1000</code> - 2x\n"
        "• <code>руль зеро 1000</code> - 35x\n\n"
        "Пример: <code>руль красное 1000</code>",
        reply_markup=cancel_reply_keyboard()
    )
...
async def roulette_play(message: Message, state: FSMContext):
    ...
    if len(parts) < 3:
        await message.answer("❌ Пример: <code>руль красное 1000</code>")
        return
...
async def coin_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "• <code>монета орёл 1000</code> - 2x\n"
        "• <code>монета решка 1000</code> - 2x\n\n"
        "Пример: <code>монета орёл 1000</code>",
        reply_markup=cancel_reply_keyboard()
    )
...
async def coin_play(message: Message, state: FSMContext):
    ...
    parts = message.text.lower().split()
    if len(parts) < 3:
        await message.answer("❌ Пример: <code>монета орёл 1000</code>")
        return
...
async def hilo_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "• Равная карта = 10x!\n\n"
        "Пример: <code>хило выше 1000</code>\n"
        "Варианты: выше, ниже, равно\n\n"
        f"💰 Введите ставку в формате: <code>хило [выше/ниже/равно] [сумма]</code>",
        reply_markup=cancel_reply_keyboard()
    )
...
async def hilo_play(message: Message, state: FSMContext):
    ...
    parts = message.text.lower().split()
    if len(parts) < 3:
        await message.answer("❌ Пример: <code>хило выше 1000</code>")
        return
...
async def main():
    ...
    print("✅ База данных сохраняется между перезапусками")
