...
def back_to_games_reply_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="◀️ Назад в игры")]],
        resize_keyboard=True
    )
...
async def dice_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "🎲 <b>Dice</b>\n\n"
        "Выбери шанс командой:\n"
        "• <code>dice 50</code> - 50% (2x)\n"
        "• <code>dice 33</code> - 33% (3x)\n"
        "• <code>dice 25</code> - 25% (4x)\n"
        "• <code>dice 10</code> - 10% (10x)\n"
        "• <code>dice 5</code> - 5% (20x)\n"
        "• <code>dice 1</code> - 1% (99x)\n\n"
...
@dp.message(GameStates.dice_bet)
async def dice_chance_input(message: Message, state: FSMContext):
    try:
...
async def crash_play(message: Message, state: FSMContext):
    ...
    # Игрок всегда выигрывает с множителем 1.5x для простоты (можно менять)
...
async def coin_play(message: Message, state: FSMContext):
    ...
    side_map = {"орёл": "heads", "орёл": "heads", "орел": "heads", "решка": "tails"}
